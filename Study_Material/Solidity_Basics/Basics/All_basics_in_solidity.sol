...
contract MyContract {
    ...
    address public immutable i_owner; // Values of immutable variables can be set only once inside the constructor
    uint public constant CONST_VALUE = 100; // Constant value declartion
    ...
    constructor() {
        ...
        i_owner = msg.sender; // Owner set to deployer of this contract
    }
    ...
    function getName() public view returns (string memory) {
        //function declaration with view modifier
        return name; //function body
    }

    function resetName() internal {
        //function declaration as internal that is it can be called inside the contract only
    ...
    function set_Name(string memory _name) public onlyOwner {
        //applying the custom modifier on the function
        name = _name;
    }
...
//declaring an enum
  enum State{
    Pending, // Follows 0 based indexing 
    Shipped,// 1
    Accepted,//2
    Rejected,//3
    Canceled//4
  ...
  State public state; //Default value is the first element listed in enum definition


 //Getter function to get current state
  function get() public returns(State){
    return state;
  }

  // Update the state by passing uint as input 
  ...
  function rejected() public{
    state = State.Rejected; //You can update to specific enum like this
  }

  
  function reset() public{
    delete state; //Resets the state to its default value that is 0
  }
    ...
    fallback()
        external
        payable
    // fallback function which is called when Ether is received without any data
    {
    ...
    function deposit(
        ...
        uint _amount // depositing ERC20 tokens
    ) public {
...
contract My2ndContract is
    MyContract // inheriting the MyContract
{
